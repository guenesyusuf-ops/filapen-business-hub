...
import {
  ...
  CheckCircle2, AlertCircle, Truck as TruckIcon, Ban, History,
} from 'lucide-react';
import {
  purchasesApi, fmtDate, fmtDateTime,
  STATUS_LABELS, PAYMENT_STATUS_LABELS, PAYMENT_METHOD_LABELS, DOCUMENT_TYPE_LABELS,
      ...
      <div className="rounded-2xl border border-gray-200/80 dark:border-white/8 bg-white dark:bg-white/[0.03] p-5">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {order.receivedAt && <Badge color="bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300">Wareneingang {fmtDate(order.receivedAt)}</Badge>}
            {order.expectedDelivery && !order.receivedAt && <span className="text-xs text-gray-500">Erwartet: {fmtDate(order.expectedDelivery)}</span>}
          </div>
          <div className="flex items-center gap-2">
            ...
            {!order.receivedAt && order.status !== 'cancelled' && (
              <button onClick={() => setOrderStatus('received')} className={btn('secondary')}>
                <TruckIcon className="h-4 w-4" /> Wareneingang
              </button>
            )}
