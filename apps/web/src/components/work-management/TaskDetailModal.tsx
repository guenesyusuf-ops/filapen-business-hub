...
import {
  ...
  CheckSquare, Square, Plus, Trash2, Download,
} from 'lucide-react';
...
export function TaskDetailModal({
  ...
  function saveDesc() {
    if (editDesc !== (task.description ?? '')) {
      onUpdate({ id: task.id, description: editDesc });
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-12 pb-12">
      ...
      <div className="relative w-full max-w-4xl max-h-[calc(100vh-6rem)] overflow-y-auto rounded-2xl bg-white dark:bg-[#1a1d2e] border border-gray-200 dark:border-white/10 shadow-2xl">
        {/* Close */}
        <button
          onClick={onClose}
        ...
        <div className="flex flex-col md:flex-row">
          ...
          {/* Sidebar */}
          <div className="w-full md:w-64 border-t md:border-t-0 md:border-l border-gray-200 dark:border-white/10 p-5 space-y-5 bg-gray-50/50 dark:bg-white/[0.02] md:rounded-r-2xl">
            {/* Status */}
            <div>
              ...
              <select
                value={task.columnId}
                onChange={(e) => onUpdate({ id: task.id, columnId: e.target.value })}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => onUpdate({ id: task.id, assigneeId: e.target.value || undefined })}
                className="w-full rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0f1117] px-2.5 py-1.5 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-400"
              >
            ...
            <div>
              ...
              <select
                value={task.priority}
                onChange={(e) => onUpdate({ id: task.id, priority: e.target.value as WmTask['priority'] })}
                className="w-full rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0f1117] px-2.5 py-1.5 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-400"
              >
